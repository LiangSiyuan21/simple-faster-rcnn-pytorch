...
class VictimFasterRCNNTrainer(nn.Module):
    """wrapper for conveniently training. return losses
    ...
    * :obj:`total_loss`: The sum of 4 loss above.
    ...
    def __init__(self, faster_rcnn, attacker=None, attack_mode=False):
        ...
        self.roi_cm = ConfusionMeter(2)
        self.meters = {k: AverageValueMeter() for k in LossTuple._fields}  # average loss
        self.adv_meters = {k: AverageValueMeter() for k in LossTupleAdv._fields}  # average loss
    ...
    def update_meters(self, losses, adv=False):
        loss_d = {k: at.scalar(v) for k, v in losses._asdict().items()}
        if not adv:
            for key, meter in self.meters.items():
                meter.add(loss_d[key])
        else:
            for key, meter in self.adv_meters.items():
                meter.add(loss_d[key])


    def reset_meters(self,adv=False):
        for key, meter in self.meters.items():
            meter.reset()
        if adv:
            for key, meter in self.adv_meters.items():
    ...
    def get_meter_data(self,adv=False):
        if adv:
            return {k: v.value()[0] for k, v in self.adv_meters.items()}
        else:
            return {k: v.value()[0] for k, v in self.meters.items()}
